...
void AST::mainHeader(const std::vector<std::string>& profileName, const std::vector<std::string>& fileName) {
    ...
    std::vector<std::string>::const_iterator namei = profileName.begin();
    
    //The original profile name
    std::string fileCpp = (*namei);
    size_t s = 0; 
    while(s != fileCpp.length()){
        if(fileCpp[s] == '_')
            fileCpp[s] = '.';
        s++;           
    }

    while(namei != profileName.end()) {
        profileString = "\nprofile " + (*namei) + "(\"" + fileCpp + "\")" + ";"; 
        child.insert(i, new AST(token, profileString));
        namei++;  
    }
...
void AST::mainReport(const std::vector<std::string>& profileName) {
    ...
    std::list<AST*>::iterator i = child.begin();
    while(i != child.end()) {
        if(((*i)->tag == "function") && ((*i)->getChild("name")->getName() == "main")) {

            std::list<AST*>::iterator blockptr = (*i)->getChild("block")->getChild("block_content")->child.begin();
            while((*blockptr)->tag != "return") {
                blockptr++;
            }
            std::list<AST*>::iterator returnptr = (*blockptr)->child.begin();
            (*returnptr)->print(std::cout);

            child.insert(returnptr, new AST(token, "std::cout << \"foo_cpp\" << std::endl;\n"));
            child.insert(returnptr, new AST(token, "\tstd::cout << \"main1_cpp\" << std::endl;\n\t"));
